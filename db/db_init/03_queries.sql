-- db_sql/03_queries.sql
-- Reference queries returning JSON payloads

-- View class info (replace :class_id with a bound value in app code)
WITH base AS (
...
) ORDER BY cl.term_id, cl.code)
FROM classes cl
LEFT JOIN rooms r ON r.id = cl.room_id
JOIN teachers t ON t.id = cl.teacher_id
WHERE t.user_id = :teacher_user_id;

-- Teacher students' performance (optional :term_id, :class_code)
WITH t_classes AS (
  SELECT cl.id FROM classes cl JOIN teachers t ON t.id = cl.teacher_id
  WHERE t.user_id = :teacher_user_id
  AND (:term_id IS NULL OR cl.term_id = :term_id)
  AND (:class_code IS NULL OR cl.code = :class_code)
),
s_rows AS (
